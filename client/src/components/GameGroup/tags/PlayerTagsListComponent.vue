...
onMounted(async () => {
  ...
  playerTagsDisplay.value = allPlayerTags.map(
    (t) => new PlayerTagRow(t.description, t.id, mapPlayerSelection(t))
  )
})

function mapPlayerSelection(tag: TagModel): PlayerTagSelection[] {
  return props.players.map((player) => {
    const playerTag = props.playerTags.some(
      (playerTag) => playerTag.playerId === player.id && playerTag.id === tag.id
    )
