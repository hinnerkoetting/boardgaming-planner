...
onMounted(async () => {
  ...
  fetchNextGamingEvents(gameGroupId, startTime).then((events) => {
    gamingEvents.value = filterDuplicateEvents(
      events
        .map((e) => expand(e))
        .flat()
...
function expand(event: GamingEvent): GamingEvent[] {
  ...
  if (event.schedule === 'ONCE') {
    ...
    while (
      recurringDate <= maxInclusiveDate.getTime() &&
      recurringDate >= minInclusiveDate.getTime()
    ) {
      const newEvent = { ...event, start: recurringDate }

      if (event.schedule === 'WEEKLY') {
        recurringDate = recurringDate + 6.048e8
...
function determineScheduleStartDate(scheduleBaseTime: Date, schedule: 'WEEKLY' | 'MONTHLY'): Date {
  ...
  const scheduleInterval = schedule === 'WEEKLY' ? 6.048e8 : 2.628e9 // 1 week in ms or 1 month in ms
  const numberOMissingSchedules = Math.floor(
    (minInclusiveDate.getTime() - scheduleBaseTime.getTime()) / scheduleInterval
  )
  const firstInclusiveDate =
    scheduleBaseTime.getTime() + (numberOMissingSchedules + 1) * scheduleInterval
